...
if($periodo==""||$periodo==null){
    $sql = "SELECT * FROM ordenestudio where pago < total and (fecha between '$fechaInicio' and '$fechaTermino') ";
}else{
    $fecha = date("Y-m-d");
    switch($periodo){
        case "dia":
            $sql = "SELECT * FROM ordenestudio where pago < total and (fecha = '$fecha') ";
            break;
        case "semana":
            $sql = "SELECT * FROM ordenestudio where pago < total and (SELECT WEEK('$fecha'))=(SELECT WEEK(ordenestudio.fecha)) ";
            break;
        case "mes":
            $sql = "SELECT * FROM ordenestudio where pago < total and (SELECT MONTH('$fecha'))=(MONTH (ordenestudio.fecha)) ";
            break;
    }
...
<h3 id="titulo" class="d-none">Lista de Ordenes de estudio sin liquidar
    <?php 
...
<table class="table table-responsive imprimir">
    <thead>
        <tr>
...
<a class="btn btn-info" href="ListaPorPagar.php?periodo=<?php echo $_GET['periodo']?>&fechainicio=<?php echo $_GET['fechainicio']?>&fechatermino=<?php echo $_GET['fechatermino']?>" id="btnImprimir" class="btn btn-info">Ver en detalle</a>
</body>
</html>
